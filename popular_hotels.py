...
def extract_hotels(location_name):
                location_name_t = location_name.encode('utf-8').strip()
                print "Extracting hotel details in location: "+location_name_t
                results=[]
                try:
...
city_details = data.map(extract_hotels).flatMap(lambda x:x).coalesce(1)
